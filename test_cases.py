...
from Kolluri_04_01 import build_generator, build_discriminator, DCGAN
...
def test_generator():
    ...
    assert isinstance(
        generator_model.layers[1], tf.keras.layers.BatchNormalization)
    ...
    assert isinstance(
        generator_model.layers[4], tf.keras.layers.Conv2DTranspose)
    assert isinstance(
        generator_model.layers[5], tf.keras.layers.BatchNormalization)
    assert isinstance(generator_model.layers[6], tf.keras.layers.LeakyReLU)
    assert isinstance(
        generator_model.layers[7], tf.keras.layers.Conv2DTranspose)
    assert isinstance(
        generator_model.layers[8], tf.keras.layers.BatchNormalization)
    assert isinstance(generator_model.layers[9], tf.keras.layers.LeakyReLU)
    assert isinstance(
        generator_model.layers[10], tf.keras.layers.Conv2DTranspose)
...
def test_discriminator():
    ...
    assert abs(before_actual[0, 0] - (-0.30697495)
               ) < 1e-5, "Discriminator output is incorrect on input"
